...
random.seed(42)
def q2f(q, k=1):
    bugs, indexs, _ = bm25_model.query(q, k)
    fixs = [fixed_document[index] for index in indexs]
    return [{'buggy_function':bug, 'fixed_function':fix} for bug, fix in zip(bugs, fixs)]
...
fixed_document = [code['fixed_function'] for code in codebase]
tokenized_doc = [code['tokenized_buggy_function'] for code in codebase]
bm25_model = bm25(tokenizer, document, tokenized_doc)
