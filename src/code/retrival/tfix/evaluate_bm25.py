...
random.seed(42)
def q2f(q, cate, k=1):
    _, indexs, _ = bm25_models[cate].query(q, k)
    return [codebase[cate][index] for index in indexs]
...
for k in codebase:
    document = [code['buggy_code'] for code in codebase[k]]
    tokenized_doc = [tokenizer.tokenize(code) for code in document]
